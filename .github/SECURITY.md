...
`SCOPUS_API_KEY` environment variable, the `scopusflow.api_key` option or an
`api_key` argument, and it is kept out of printed output and request dumps.
Please keep your own key in `~/.Renviron`, well away from your scripts, and never
paste it into an issue, a pull request or a discussion.
