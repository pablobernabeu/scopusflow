...
can, working within the documented limits rather than around them. This
is the same approach `rscopus` takes. The quota and rate-limit headers
are parsed by `scopus_quota()`, transient failures such as HTTP 429 and
the 5xx range are retried with back-off that honours `Retry-After`, and
any single query is capped at 5000 records with a warning that suggests
