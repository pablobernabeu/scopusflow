...
* [`run_app()`] launches a local, code-free Shiny app for building a search,
  ...
  counts-in-label controls, a quota estimate and a CSV export) and a *Demo mode*,
  on by default, that synthesises records and a comparison so the whole workflow
  can be explored with no key and no network.
