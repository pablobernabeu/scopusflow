...
This vignette is fully reproducible **without a Scopus API key**: it uses a small
static fixture bundled with the package so the whole workflow can be shown
offline. The few steps that genuinely require the API are shown but not run.

## 1. Plans: describe a search before running it

A *plan* separates describing a search from executing it. Plans are inspectable,
saveable and version-controllable, and can be partitioned (for example by year)
so large retrievals stay under the API's `start < 5000` ceiling and can be cached
and resumed.
...
## 2. Sizing and fetching (live)

With a key configured, you size a search cheaply and then execute the plan,
optionally caching each cell so an interrupted run resumes without re-spending
quota. These contact the API, so they are not evaluated here:
...
Whether records come from the API or from a fixture, they share one stable
schema. Here we load the bundled fixture to continue offline:

```{r}
fixture <- system.file("extdata", "scopus_page.json", package = "scopusflow")
parsed  <- jsonlite::fromJSON(fixture, simplifyVector = FALSE)
records <- scopus_records(parsed[["search-results"]], query = "TITLE(example)")
records
```

## 4. DOIs and change tracking
...
You can write the DOIs to an explicit path (never implicitly):
...
## 5. Comparing topic trends

`scopus_compare_topics()` issues one count request per term per year, so it needs
the API. Its output has a fixed shape, which we mimic here to show the plot:
...
`scopus_error`, so a workflow can respond programmatically:

```{r eval = FALSE}
tryCatch(
  scopus_fetch("..."),
  scopus_error_no_key     = function(e) message("No API key configured."),
  scopus_error_rate_limit = function(e) message("Rate limited; back off."),
