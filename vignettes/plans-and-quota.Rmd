...
the plan, so pointing a second, different plan at the same directory would pair
its cells with the first plan's checkpoints. `scopus_fetch_plan()` compares each
checkpoint's recorded query with the plan cell before loading it and refetches
on a mismatch, but the clean arrangement is a separate directory per plan.
