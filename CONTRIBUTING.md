...
closes it again once the check passes. If a breaking change is found and
an `ANTHROPIC_API_KEY` repository secret is configured, the workflow
first asks the Claude Code action to attempt a fix and open a pull
request, which the maintainer then reviews; without the secret it simply
flags the issue. The workflow can also be run on demand from the Actions
tab.
